...
def main():
    ...
    parser.add_argument('--model', type=str, default="output_models/Qwen2-7B-rait-1/merge")
    ...
    with open(f"data/Qwen2-7B/split_data/rait_1/indomain_train.json", 'r') as f:
        data = json.load(f)
    ...
    for subject in tqdm(data.keys(), desc="Subjects"):
        ...
        for sample in tqdm(subject_samples, desc="Samples", leave=False):
            ...
            # 1) 做推理
            pred_choice, probs_5,full_input = inference(
                tokenizer=tokenizer,
                model=model,
                sample=sample,
                subject=subject,
            ...
            # 2) correctness
            if (pred_choice == correct_answer and sample["kb"]["Correctness_after_threshold"] == "Unknown") or (sample["kb"]["Correctness_after_threshold"] == "Known"):
                correctness_after_correctness_change = "Known"
            else:
                correctness_after_correctness_change = "Unknown"

            # 3) entropy
            answer_idx = {"A":0,"B":1,"C":2,"D":3,"N":4}[pred_choice]
            answer_prob = probs_5[answer_idx]
            ent = entropy(probs_5 + 1e-12)
            ...
            sample_eval = {
                ...
                "kb": {
                    "Final_Correctness": {
                        "result": correctness_after_correctness_change,
                    },
                    "Certainty": {
                        "entropy": float(ent),
                    }
        ...
        vanilla = [x for x in subject_data if x["kb"]["Final_Correctness"]["result"] == "Known"]
        idk = [x for x in subject_data if x["kb"]["Final_Correctness"]["result"] == "Unknown"]
        ...
        n_idk = int(0.75 * len(idk_sorted))

        vanilla_selected = vanilla_sorted[:n_van]
        idk_selected = idk_sorted[-n_idk:]

        rait_subject_data = vanilla_selected + idk_selected
        random.shuffle(rait_subject_data)
        
        results[subject] = rait_subject_data
    

    # 保存评估结果
    save_dir = "data/Qwen2-7B/traindata/rait_2"
    save_file = "indomain_train.json"
